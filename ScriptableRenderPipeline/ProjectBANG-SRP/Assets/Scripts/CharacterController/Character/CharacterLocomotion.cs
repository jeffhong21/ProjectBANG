namespace CharacterController
{
    ...
    public class CharacterLocomotion : MonoBehaviour
    {
        ...
        private float m_InputMagnitude, m_InputAngle, m_YawRotation;
        //[SerializeField, DisplayOnly]
        ...
        private void Update()
		{
            ...
            m_Animator.SetFloat(HashID.InputMagnitude, m_InputMagnitude, 0.1f, m_DeltaTime);
        ...
        private void UpdateMovement()
        {
            ...
            //m_Rigidbody.AddForce(m_MoveDirection * m_DeltaTime, ForceMode.VelocityChange);
		...
		//{
  //          if(collision.gameObject.isStatic == false){
  //              var rb = collision.rigidbody;
  //              if(rb){
  //                  //rb.AddForce(collision.)
  //                  var force = (m_Velocity.sqrMagnitude * 2) * m_Mass;
  //                  var position = collision.GetContact(0).point + m_Animator.bodyPosition.y * m_Transform.up;
  //                  //Debug.LogFormat("Collided with {0}.  Applying force of {1}", collision.gameObject, force);
  //                  //Debug.DrawRay(collision.GetContact(0).point, collision.GetContact(0).normal, Color.red, 1f);
  //                  //Debug.Break();
  //                  //rb.AddExplosionForce(force, position, m_CapsuleCollider.radius * 2, 0, ForceMode.Impulse);
  //                  //rb.AddForce(collision.GetContact(0).normal * force, ForceMode.Impulse);
  //              }
  //          }
        ...
        private void OnGUI()
        {
            if (Application.isPlaying)
            {
                debugMsgs = new string[]{
                    ...
                    //string.Format("Input Vector: {0}", m_InputVector),
                    //string.Format("Relative Input: {0}", m_RelativeInputVector),
                    string.Format("Pivot Position: {0}", m_Animator.pivotPosition),
                    string.Format("Left IK Pos: {0}", m_Animator.GetIKPosition (AvatarIKGoal.LeftFoot)),

...
public class DamageArea : MonoBehaviour
{
    ...
    {
        var hitCollision = c.gameObject.GetComponent<CharacterHealth>();
        if (hitCollision == null)
            return;

        charactersInside.Add(new CharacterInfo { characterHealth = hitCollision, nextDamageTick = 0 });
    }
    ...
    void OnTriggerExit(Collider c)
    {
        ...
        {
            if (charactersInside[i].characterHealth == hitCollision)
            {

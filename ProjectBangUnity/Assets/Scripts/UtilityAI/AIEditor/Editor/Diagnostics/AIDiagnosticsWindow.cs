            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(new GUIContent("Delegate"), EditorStyles.toolbarButton, GUILayout.Width(75)))
                {
                    if(_visualizedEntity != null && !string.IsNullOrEmpty(_lastOpenAI)){
                        var field = typeof(AINameMapHelper).GetField(_lastOpenAI, System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
                ...
                if (GUILayout.Button(new GUIContent("Debug Clients"), EditorStyles.toolbarButton, GUILayout.Width(75)))
                {
                    if(AIManager.allClients != null){
                        string message = string.Format("All Clients\n");
                        var index = 0;
                        ...
                        message += string.Format("Total Clients: {0}\n", index);
                        Debug.Log(message);
                    }

namespace UtilityAI
{
    ...
    public class EntityAISteering : MonoBehaviour
    {
        ...
        float GetDistanceRemaining(bool useAgentPath = true)
        {
            float distance = 0.0f;
        ...
        bool HasReachedDestination(bool useAgentPath = true){
            if(useAgentPath)
                return GetDistanceRemaining(useAgentPath) <= arrivalDistance && agent.pathPending == false;
            else
                return GetDistanceRemaining(useAgentPath) <= arrivalDistance;
        }
